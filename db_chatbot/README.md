...
The current project centers on `api_data/*_selected.json` and the normalized build in
...
2. Keep `*_selected.json` as the main local source artifact.
...
5. Run the chatbot against either:
   - `api_selected` mode: reads `api_data/*_selected.json` directly
...
There is also an older Excel-contract pipeline:

```bash
python db_chatbot/scripts/build_contract_workbook_from_api_excels.py
python db_chatbot/scripts/load_validate_data.py
```

That path writes to `db_chatbot/build/`, but the active repo workflow is the API-selected JSON path above.
...
```bash
python db_chatbot/chat_app.py --source-mode api_selected --query "교촌치킨 요약해줘"
```
...
```bash
python db_chatbot/chat_app.py --source-mode api_selected
```
...
```bash
python testing/run_db_missing_eval.py --show-response
```

Test sets currently include:

- `testing/db_missing_question.json`
- `testing/inference_smoke_hard.json`
